...
- **[@workspace/logger](../packages/logger)** - Structured request and application logging with Pino
...
- **Structured Logging** - Request and application logging with Pino via `@workspace/logger` (structured JSON in production, pretty output in dev)
- **Per-request Logger** - `pino-http` attaches a request-scoped logger (`req.log`) with a request id for correlated logs
- **Real Client IP** - `trust proxy` resolves `req.ip` behind the ingress; the IP and authenticated user are attached to each request log
- **Health Checks** - Readiness and liveness endpoints (excluded from request logs to reduce noise)
